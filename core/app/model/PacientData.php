...
class PacientData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (name,lastname,gender,day_of_birth,address,phone,email,sick,medicaments,alergy,created_at,notes,eps_type,eps,city,depto,job,cellphone,id_type,id_doc) ";
		$sql .= "value (\"$this->name\",\"$this->lastname\",\"$this->gender\",\"$this->day_of_birth\",\"$this->address\",\"$this->phone\",\"$this->email\",\"$this->sick\",\"$this->medicaments\",\"$this->alergy\",$this->created_at,\"$this->notes\",\"$this->eps_type\",\"$this->eps\",\"$this->city\",\"$this->depto\",\"$this->job\",\"$this->cellphone\",\"$this->id_type\",\"$this->id_doc\")";
		Executor::doit($sql);
	}
